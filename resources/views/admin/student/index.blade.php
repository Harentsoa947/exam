...
<div class="bg-white h-full rounded-md">
    <div class="bg-white sticky top-0">
        ...
        @endif
        <div class="flex gap-3 border-b-2 border-black/10">
            <a href="{{ route('admin.student.index') }}"
                class="p-1 inline-block hover:text-[rgb(250,131,51)] {{ !request('categorie_id') ? 'text-[rgb(250,131,51)] font-semibold' : '' }}">
                Tous
            </a>
            @foreach ($categories as $categorie)
                <a href="{{ route('admin.student.index', ['categorie_id' => $categorie->id]) }}"
                    class="p-1 inline-block hover:text-[rgb(250,131,51)] {{ request('categorie_id') == $categorie->id ? 'text-[rgb(250,131,51)] font-semibold' : '' }}">
                    {{ $categorie->nom }}
                </a>
            @endforeach
        </div>
