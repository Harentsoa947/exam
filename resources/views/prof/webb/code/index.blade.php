        ...
        <div class="">
            <div class="flex justify-between items-start my-3">
                ...
                <div class=" flex justify-end mt-4">
                    <a href="{{route('prof.examen.web.code.create', $examen->id)}}" class="p-1 px-3 inline-block rounded-md bg-rouge ">
                        Créer nouveau exercice
                    </a>
                </div>
            </div>
           
            @forelse($codeWebs as $index => $codeWeb)
                <div class="py-2 flex gap-7 justify-between border-b border-black/10 my-2">
                    ...
                    <div class="flex-1">
                        <h3 class="text-xl  font-semibold">{{ $codeWeb->titre }}</h3>
                        ...
                        <div class="flex gap-3">
                            ...
                            <div class="flex text-sm">
                                Question: <span class="border border-black/10 rounded-full px-3 inline-block text-vert">{{ $codeWeb->code_web_questions_count }}</span>      
                            </div>
                        </div>
                    </div>
                    <div class="flex gap-4">
                        <a href="{{route('prof.examen.web.code.question.index', [$examen->id, $codeWeb->id])}}" class="text-vert">
                            <i class="fa-solid fa-arrow-up-right-from-square"></i>
                        </a>
                        <a href="" class="text-black/60">
                            <i class="fa-solid fa-pen"></i>
                        </a>
                        <form action="{{ route('prof.examen.web.code.destroy', [$examen->id, $codeWeb->id]) }}" method="POST" onsubmit="return confirm('Supprimer {{ $codeWeb->titre }} ? Cette action supprimera aussi toutes ses questions.')">
                            @csrf
                            @method('DELETE')
