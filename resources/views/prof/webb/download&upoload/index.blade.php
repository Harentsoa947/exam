    ...
    <div class="py-3">
        ...
        <div class="">
            <div class="flex justify-between items-start my-3">
                <div class="w-[70%]"> 
                    <h3 class="font-semibold text-base pb-1">Download & Upload fichier</h3>
                ...
                <div class=" flex justify-end mt-4">
                    <a href="{{route('prof.examen.web.download-upload.create', $examen->id)}}" class="p-1 px-3 inline-block rounded-md bg-rouge ">
                        Créer nouveau exercice
                    </a>
                </div>
            </div>
           
            @forelse($fichierWebs as $index => $fichierWeb)
                <div class="py-2 flex gap-7 justify-between border-b border-black/10 my-2">
                    ...
                    <div class="flex-1">
                        ...
                        <p>{{ $fichierWeb->description }}</p>
                        <div class="flex gap-3">
                            <div class="flex text-sm">
                                Durée: <span class="border border-black/10 rounded-full px-3 inline-block text-rouge">Libre</span>
                            </div>
                            <div class="flex text-sm">
                                Question: <span class="border border-black/10 rounded-full px-3 inline-block text-vert">{{ $fichierWeb->relier_web_questions_count }}</span> 
                                
                            </div>
                        </div>
                    </div>
                    <div class="flex gap-4">
                        <a href="{{route('prof.examen.web.download-upload.qeustion.index', [$examen->id, $fichierWeb->id])}}" class="text-vert">
                            <i class="fa-solid fa-arrow-up-right-from-square"></i>
                        </a>
                        <a href="" class="text-black/60">
                            <i class="fa-solid fa-pen"></i>
                        </a>
                        <form action="{{ route('prof.examen.web.download-upload.destroy', [$examen->id, $fichierWeb->id]) }}" method="POST" onsubmit="return confirm('Supprimer {{ $fichierWeb->titre }} ? Cette action supprimera aussi toutes ses questions.')">
                            @csrf
                            @method('DELETE')
