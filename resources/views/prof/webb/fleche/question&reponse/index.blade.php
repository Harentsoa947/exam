    ...
    <div class="py-3">
        ...
        <div class="py-3">
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('prof.examen.web.fleche', $examen->id) }}" 
                    class="  w-7 h-7 rounded-sm bg-vert flex justify-center items-center text-white">
                    <i class="fa-solid fa-chevron-left"></i>
                </a>
                <span>Détail — Compléter le pointillé</span>
            </div>
            <div class="border border-black/3">
                <div class="flex justify-between gap-5 border-b-2 bg-black/3 border-black/10 p-2">
                    ...
                    <div class="">
                        <a href="{{ route('prof.examen.web.fleche.question.create', [$examen->id, $relierWeb->id]) }}" 
                ...
                <div class="p-4 border-b border-black/5">
                    <div class="flex justify-between">
                        <h3 class="font-bold">
                            {{ $loop->iteration }} - {{ $question->enonce }}
                        </h3>
                        <div class="flex gap-5">
                            ...
                            <div class="flex gap-3">
                                ...
                                <form action="{{route('prof.examen.web.fleche.question.destroy', [$examen->id, $relierWeb->id, $question->id])}}"
                    ...
                    <div class="grid grid-cols-2 gap-15 mt-2">
                        <div>
                            <p class="text-black/50 text-sm">Colonne gauche</p>
                            @foreach ($question->paires as $paire)
                                <div class="py-1 flex justify-between gap-2">
                                    <div class="flex-1">
                                        - {{ $paire->element_left }} 
                                    </div>
                                     <span class="w-7 h-6 flex justify-center items-center rounded-md bg-black/30">{{ $paire->order_left }}</span> 
                                </div>
                            @endforeach
                        </div>
                        <div>
                            <p class="text-black/50 text-sm">Colonne droite</p>
                            @foreach ($question->paires as $paire)
                                <div class="py-1 flex justify-between gap-2 text-left">
                                     <span class="w-7 h-6 flex justify-center items-center rounded-md bg-vert">{{ $paire->order_right }}</span>
                                     <div class="flex-1">
                                        - {{ $paire->element_right }}
                                    </div> 
                                </div>
        ...
        {{-- <div class="py-5">
            @forelse($questions as $index => $question)
                <div class="border-b border-black/3 my-2">
                    @if($question->image)
                    ...
                    <div class="flex gap-5 justify-between py-2">
                        ...
                        <div class="flex gap-3 items-center">
                            ...
                            <a href="{{ route('prof.examen.web.pointiller.question.edit', [$examen->id, $pointillerWeb->id, $question->id]) }}" class="text-vert">
                                <i class="fa-solid fa-pen"></i>
                            </a>
                            <form action="{{ route('prof.examen.web.pointiller.question.destroy', [$examen->id, $pointillerWeb->id, $question->id]) }}" method="POST" onsubmit="return confirm('Supprimer cette question ?')">
                                @csrf
                                @method('DELETE')
